...
export default function Board() {
  const [deck, setDeck] = React.useState<deckInfo>([]);
  ...
  function initializeGame() {
    const theDeck = initializeDeck();
    const top4Cards = getTopXCardsOfDeck(4, theDeck);
    const playerStartingHand = top4Cards.slice(0, 2);
    setPlayerCards(playerStartingHand);
    const dealerStartingHand = top4Cards.slice(2);
    setDealerCards(dealerStartingHand);
    theDeck.splice(theDeck.length - 4, 4);
    setDeck(theDeck);
    const blackJackMessage = checkForBlackJack(
      playerStartingHand,
      dealerStartingHand
    );
    ...
    <>
      ...
      <div id="buttonsAndMessage">
        <input
          ...
          type="button"
          value="Hit"
          disabled={isGameOver || deck.length < 1}
